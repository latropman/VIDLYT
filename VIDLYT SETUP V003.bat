...
echo %ESC%[38;2;198;198;108m^> Checking yt-dlp...%ESC%[0m
if not exist "%YTDLP%" (
    echo %ESC%[38;2;198;108;108m^> yt-dlp.exe not found!%ESC%[0m
    pause
    exit /b
)
<nul set /p=%ESC%[1A
<nul set /p=%ESC%[2K
echo %ESC%[38;2;108;198;183m^> yt-dlp ready%ESC%[0m
...
if "%URL%"=="" (
    echo Clipboard is empty or does not contain a link.
    set /p URL=Please paste the link manually and press Enter:
    if "%URL%"=="" (
...
<nul set /p=%ESC%[1A
<nul set /p=%ESC%[2K
echo %ESC%[38;2;198;198;108mSelect quality:%ESC%[0m
echo [1] BEST VIDEO
echo [2] BEST AUDIO
echo [3] 4K (2160p)
echo [4] 2K (1440p)
